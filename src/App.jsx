    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
      ...
      >
        <Heart className="text-white fill-current w-12 h-12 md:w-16 md:h-16 drop-shadow-lg" />
         ...
         <p className="text-pink-600 font-serif font-bold text-xs md:text-sm tracking-widest uppercase text-center">{name}'s Memory</p>
      <p className="absolute bottom-10 text-white/70 font-mono text-[8px] uppercase tracking-[0.4em] z-10 font-bold animate-pulse">Swipe to Turn Pages</p>
    </div>
  </div>
);
...
export default function App() {
  ...
  const name = "ACAA"; 
  const surpriseSequence = ["3", "2", "1", "HAPPY", "BIRTHDAY", "TO","YOU", name];
  ...
  // Daftar 4 foto utama dari buku untuk kolase akhir
  const finalPhotos = [
    "src/assets/1.jpeg",
    "src/assets/2.jpeg",
    "src/assets/3.jpeg",
  ...
  useEffect(() => {
    if (phase === 'sequence') {
      if (sequenceIndex < surpriseSequence.length) {
        const timer = setTimeout(() => setSequenceIndex(sequenceIndex + 1), 850);
  ...
  const collagePhotos = Array.from({ length: 24 }).map((_, i) => {
    ...
    const y = - (220 * Math.cos(angle) - 80 * Math.cos(2 * angle) - 35 * Math.cos(3 * angle) - 15 * Math.cos(4 * angle));
    return { x, y, id: i };
  });
  ...
  return (
    <div className="min-h-screen text-white font-sans overflow-hidden bg-black select-none flex flex-col items-center justify-center w-full">
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'intro' && (
          <motion.div key="intro" exit={{ opacity: 0, scale: 0.9 }} className="z-10 text-center px-6">
            <motion.div 
            ...
            >
              <div className="bg-black/80 backdrop-blur-md rounded-[22px] p-8 flex flex-col items-center border border-white/10">
                <Sparkles size={44} className="text-yellow-400 mb-4 animate-pulse" />
                <h1 className="text-2xl font-black tracking-[0.2em] uppercase mb-2">HBD ACAA</h1>
                <p className="text-pink-400 text-xs font-mono uppercase tracking-widest">Let's Open Your BDay Present</p>
              </div>
            </motion.div>
            <motion.button
        ...
        {phase === 'sequence' && (
          <motion.div key="sequence" exit={{ opacity: 0 }} className="z-20 text-center">
            <AnimatePresence mode="wait">
              <motion.div
                key={sequenceIndex}
        ...
        {phase === 'heartPulse' && (
          <motion.div key="pulse" exit={{ opacity: 0 }} className="z-10">
            <motion.div animate={{ scale: [1, 1.4, 1] }} transition={{ repeat: Infinity, duration: 0.5 }}>
              <Heart size={140} className="text-pink-500 fill-current drop-shadow-[0_0_50px_rgba(236,72,153,0.7)]" />
            </motion.div>
          </motion.div>
        )}

        {phase === 'character' && (
          <motion.div key="char" exit={{ opacity: 0 }} className="z-10 text-center">
            <motion.div 
        ...
        {phase === 'reveal' && (
          <motion.div key="reveal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative w-full h-full flex flex-col items-center justify-center p-4">
            ...
            <div className="z-30 mb-8 max-w-sm w-full min-h-[70px] flex items-center justify-center text-center px-2">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <Mail className="absolute -top-3 -right-3 text-pink-500 bg-white rounded-full p-1 shadow-md" size={24} />
                ...
                >
                  <motion.div 
                    ...
                    animate={{ 
                      ...
                      scale: flippedLeaves > 0 ? 1 : 0.8,
                      visibility: flippedLeaves > 0 ? 'visible' : 'hidden'
                    }}
                    transition={{ duration: 0.4 }}
                    className="w-1/2 h-full bg-pink-50 rounded-l-lg border-r border-gray-200 flex items-center justify-center p-2 overflow-hidden z-0"
                  >
                     <img src="https://picsum.photos/seed/mem_start/600/800" className="w-full h-full object-cover rounded shadow-md border border-white" alt="Memori Start" />
                  </motion.div>
                  ...
                  <motion.div 
                    ...
                    animate={{ 
                      ...
                      scale: flippedLeaves === 3 ? 1 : 0.8,
                      visibility: flippedLeaves === 3 ? 'visible' : 'hidden'
                    }}
                    transition={{ duration: 0.4 }}
                    className="w-1/2 h-full bg-white rounded-r-lg p-2 flex items-center justify-center border-l border-gray-100 overflow-hidden relative z-0"
                  >
                    <img src="https://picsum.photos/seed/mem_final/600/800" className="w-full h-full object-cover rounded shadow-md border border-white" alt="Memori Final" />
                  </motion.div>

                  <Leaf index={3} isFlipped={flippedLeaves > 2} totalLeaves={3}>
                    <img src="https://picsum.photos/seed/mem_4/600/800" className="w-full h-full object-cover rounded" alt="Memori 4" />
                    <div className="w-full h-full flex flex-col items-center justify-center bg-pink-700 rounded-r-lg shadow-inner overflow-hidden relative">
                       <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle,white_1px,transparent_1px)] bg-[size:20px_20px]" />
                       <Heart size={50} className="text-white fill-current mb-4 animate-pulse z-10" />
                       <h3 className="text-white font-black text-xl tracking-[0.3em] uppercase z-10">Forever</h3>
                       <p className="text-white/40 text-[10px] mt-6 italic z-10">Swipe left to end</p>
                    </div>
                  </Leaf>

                  <Leaf index={2} isFlipped={flippedLeaves > 1} totalLeaves={3}>
                    <img src="https://picsum.photos/seed/mem_2/600/800" className="w-full h-full object-cover rounded" alt="Memori 2" />
                    <img src="https://picsum.photos/seed/mem_3/600/800" className="w-full h-full object-cover rounded" alt="Memori 3" />
                  </Leaf>

                  <Leaf index={1} isFlipped={flippedLeaves > 0} totalLeaves={3}>
                    <NotebookCover 
                      onClick={() => setFlippedLeaves(1)} 
                      photoUrl="https://picsum.photos/seed/cover_anita/600/800" 
                      name={name}
                    />
                    <img src="https://picsum.photos/seed/mem_1_alt/600/800" className="w-full h-full object-cover rounded" alt="Memori 1" />
                  </Leaf>
            ...
            <div className="mt-12 flex flex-col items-center gap-4">
              ...
              >
                {flippedLeaves === 3 ? "Swipe left to see our love heart" : "Swipe horizontally to flip"}
              </motion.p>
            </div>
          </motion.div>
        )}
        ...
        {phase === 'collage' && (
          <motion.div key="collage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative w-full h-full flex items-center justify-center">
            <div className="relative w-full h-full flex items-center justify-center">
              {collagePhotos.map((p, i) => (
                <motion.div
                  key={p.id}
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{ opacity: 1, scale: 1, x: p.x, y: p.y, rotate: (i % 2 === 0 ? 10 : -10) }}
                  transition={{ delay: i * 0.05, type: 'spring' }}
                  className="absolute w-28 h-28 md:w-40 md:h-40 border-4 border-white rounded-lg shadow-2xl overflow-hidden bg-white"
                >
                  <img src={finalPhotos[i % 4]} className="w-full h-full object-cover" alt="Memory" />
                </motion.div>
              ))}
              ...
              >
                <h2 className="text-3xl md:text-5xl font-black text-pink-500 uppercase tracking-tighter leading-none drop-shadow-[0_0_20px_rgba(236,72,153,0.4)]">Happy Birthday<br/>{name}</h2>
                <p className="text-pink-300 text-[10px] mt-4 font-mono tracking-[0.5em] font-bold uppercase">Always & Forever</p>
              </motion.div>
            </div>
          </motion.div>
        )}
